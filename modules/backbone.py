...
class ResTransformer_num(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, images, layer_num=3):
        feature = self.resnet(images, layer_num=layer_num)
        n, c, h, w = feature.shape # [66, 128, 8, 32]
        feature = feature.view(n, c, -1).permute(2, 0, 1)
        feature = self.pos_encoder(feature)
        feature = self.transformer(feature)
        feature = feature.permute(1, 2, 0).view(n, c, h, w)
    ...
    def BGforward(self, images, layer_num=3):
        feature = self.resnet(images, layer_num=layer_num)
        n, c, h, w = feature.shape
        feature = feature.view(n, c, -1).permute(2, 0, 1)
        feature = self.pos_encoder(feature)
