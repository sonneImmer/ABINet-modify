...
class ImageDatasetWithEmbedding(Dataset):
    """`ImageDataset` read data from LMDB database."""
    ...
    def get(self, idx):
        with self.env.begin(write=False) as txn:
            ...
            try:
                ...
                logging.info(f'Corrupted image is found: {self.name}, {idx}, {label}, {len(label)}')
                return self._next_image(idx)
        return image, label, embed_vec, idx
    ...
    def __getitem__(self, idx):
        image, text, embed_vec, idx_new = self.get(idx)
        ...
        if self.return_idx:
            y = [label, length, embed_vec, idx_new]
        else:
            y = [label, length, embed_vec]
        return image, y

class ImageDatasetWithWPEmbedding(Dataset):
    """`ImageDataset` read data from LMDB database."""
    ...
    def get(self, idx):
        with self.env.begin(write=False) as txn:
            image_key, label_key, embed_key = f'image-{idx + 1:09d}', f'label-{idx + 1:09d}', f'wpembed-{idx + 1:09d}'
            try:
                label = str(txn.get(label_key.encode()), 'utf-8').strip()  # label
                if not set(label).issubset(self.character):
                    return self._next_image(idx)
